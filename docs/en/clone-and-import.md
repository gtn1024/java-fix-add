...
`git clone https://github.com/ByteLegendQuest/java-fix-add`

A project directory named `java-fix-add` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-add` directory which is created just now.
